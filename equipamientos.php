          ...
          <table class="tabla">
            ...
            <tbody>
              ...
              <?php foreach ($equipamientos as $equipamiento): ?>
                <?php
                  $estadoVisible = (int) $equipamiento["estado"] === 1 ? "Disponible" : "Inactivo";
                  $claseEstado = (int) $equipamiento["estado"] === 1 ? "estado-disponible" : "estado-inactivo";
                ?>
                <tr>
                  <td><strong><?= escapar($equipamiento["nombre"]) ?></strong></td>
                  <td><?= escapar($equipamiento["descripcion"]) ?></td>
                  <td><span class="estado
                   <?= escapar($claseEstado) ?>"><?= escapar($estadoVisible) ?></span></td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
